...
def launching_kernel(reads, genome, chosen_device_parameters, no_of_reads, read_len, no_of_outputs_per_read, e, tally, SA, F, share_per_device, device_choices):
	
	program_file = open('CORAL_kernel.cl', 'r')
	program_text = program_file.read()
	program_file.close()	
	no_of_outputs_per_read = np.uint32(no_of_outputs_per_read)
	word_size = 32
	q_gram_len = np.uint32(math.floor(read_len/(e+1)))	
	...
	program_compilation_options.append(temp_text)
	temp_text = "-DQGRAMS_IN_READ=" + str(math.floor(read_len/q_gram_len))  #str(math.floor(read_len/q_gram_len)
	program_compilation_options.append(temp_text)
	temp_text = "-DGENOME_LEN=" + str(len(genome))
	program_compilation_options.append(temp_text)
	...
	program_compilation_options.append(temp_text)
	temp_text = "-DMIN_QGRAM_LEN=" + str(math.floor(read_len/(e+1))-1)  # minimum q-gram length - 1 (minus one), minus one just to prevent an extra computation
	program_compilation_options.append(temp_text)
	temp_text = "-DEXCESS_BASE=" + str(read_len - math.floor(read_len/(e+1))*(e+1))  # minimum q-gram length - 1 (minus one)
	program_compilation_options.append(temp_text)
	print('-----------------------------------------------------------------------')
...
def Write_Output(cand_locs_per_read, strand, mapped_endpos_read, device_choices, sequences, sequence_names, no_of_outputs_per_read, filename, n):
	...
	arr = []
	read_number = 0
	with open(filename+'.coral','w') as f:
		f.write(heading)
		# f.write('\t\t'.join(heading[0:]) + '\n')
		# f.write("{:25}{:^10}{:^22}{:^8}{:^10}{:^150}".format(*heading)+'\n')
		for i in device_choices:
...
def main():	
	...
	parser = argparse.ArgumentParser(description="CORAL : Cross-platform Opencl based Read ALigner. This tool maps short-reads to reference genome.")
	...
	# parser.add_argument("CPU_or_GPU", help="Select the target platform. 0-CPU and 1-GPU", type=int, default=0)
	parser.add_argument("Outputs", help="Maximum number of mappings allowed per read. Range:[1:3500]", type=int, default=100)	
	#---To be deleted later---------------------------------------------------------------------------
	# parser.add_argument("OriPos", help="Text file containing original positions of reads (simulated)", type=str)
	# parser.add_argument("OriStrand", help="Text file containing original strands of reads (simulated)", type=str)
	#---------------------------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------
	# Part to be deleted later.
	#--------------------------------------------------------------------	
	# with open(args.OriPos,'r') as f:
	# 	original_pos = f.read()
	# original_pos = original_pos.split('\n')
	# original_pos = [int(i) for i in original_pos[:-1]]
	
	# with open(args.OriStrand,'r') as f:
	# 	original_strand = f.read()
	# original_strand = original_strand.split('\n')
	# original_strand = [i for i in original_strand[:-1]]
	#-------------------------------------------------------------------
	...
	# print('{:<30}{:^3}{:.2f}'.format('Total memory usage',':',total_memory_required/(1024*1024*1024)),'GB')
	#-------------------------------------------------------------------
	strand_of_read, mapped_endpos_read, cand_locs_per_read = opencl(genome, reads, n, e, device_choices, no_of_reads, no_of_outputs_per_read, tally, SA, F, total_memory_required, share_per_device)
	print('-----------------------------------------------------------------------')
